...
- ~~`store.ts` recipes: partial-failure states (init succeeded, config write failed) leave initialized-but-unregistered stores — acceptable, document the ceiling (B08).~~ ✅ Documented as a `ponytail:` ceiling on `Store.create`: retry is the recovery (pass init idempotent for same key); full rollback would risk deleting pre-existing user directories. `Store.add` has no partial state (validation is read-only).
...
- ~~`AppSidebar.vue`: filesystem-watcher/polling infra (setInterval + service imports) embedded in a component; sidebar never renders `treeStore.error` so failed loads look like an empty store (B04).~~ ✅ Fixed: polling moved into the entry-tree store (app-scoped singleton, idempotent re-arm — no double-poll); re-arm now keys off the reactive `activeStore.storePath` ref (`Pass.storePath` is a plain class property, so the old component watch never re-fired after mount — store switches silently kept polling the previous store); `Watcher.watch` arming failure now logged; per owner direction the watcher fires on **any** change in the store tree (recursive, `.git` internals excluded) rather than `.gpg-id` only. Error rendering: covered by App.vue's global relay (toasts `treeStore.error` then nulls it), so a sidebar branch could never render — not duplicated. Live watcher behavior still needs manual verification on the Test.vue page.
