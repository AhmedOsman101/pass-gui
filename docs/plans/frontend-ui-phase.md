...
- Phase 02 types exist: `ReadinessState`, `ReadinessIssue`, `ReadinessSnapshot`, `ReadinessIssueCode`
...
- `main.ts` no longer blocks on GPG/pass init promises
...
- `neu.execCmd()` no longer throws on non-zero exit (from Phase 03 Step 0)
...
**Mapping**:
- `PASS_NOT_FOUND`/`PASS_VERSION_UNSUPPORTED`/`GPG_NOT_FOUND` → "Missing Dependencies" icon
- `GPG_NO_SECRET_KEYS` → "GPG Not Initialized" icon
- `ACTIVE_STORE_UNDEFINED`/`STORE_CONFIG_MISSING`/`STORE_PATH_NOT_FOUND` → "Store Not Found" icon
- `STORE_PATH_NOT_DIRECTORY`/`STORE_GPG_ID_MISSING`/`STORE_GPG_ID_EMPTY`/`STORE_RECIPIENT_*`/`STORE_BEHAVIORAL_CHECK_FAILED` → "Store Invalid" icon
...
- `DEPENDENCIES_MISSING`: "Install pass: `apt install pass` / `brew install pass`"
- `GPG_NOT_INITIALIZED`: "Generate GPG keys: `gpg --full-generate-key`"
- `STORE_NOT_FOUND`: "Create a store: `pass init <your-key-id>`"
- `STORE_INVALID`: "Check `.gpg-id` and ensure recipients match keyring"
