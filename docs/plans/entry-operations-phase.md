...
- Phase 02 readiness types exist: `ReadinessState`, `ReadinessIssue`, `ReadinessSnapshot`, `ReadinessIssueCode` at `client/src/types/readiness.ts`
...
- `StoreValidationError` and `STORE_ERROR_CODES` exist at `client/src/lib/errors.ts`
...
- `neu.execCmd()` currently throws `CommandFailedError` on non-zero exit code (to be fixed in Step 0)
- **`pass.execScoped()` takes `(args, options?)`** — args first, options second. Example: `pass.execScoped(["ls"], { envs: { PASSWORD_STORE_DIR: storePath } })`
...
**Fix**: Remove the throw on non-zero exit, always return `Ok(result)`. Then update the single existing caller that only checks `result.isError()` without also checking `result.ok.exitCode !== 0` — this is `validateBehavior()` in `store-validation.ts`.
...
All `pass.execScoped()` calls use `args` as FIRST argument, `options` as SECOND: `pass.execScoped(["ls"], { envs: { PASSWORD_STORE_DIR: storePath } })`.
