...
**Note**: `main.ts` currently blocks on `gpgInitialized` and `passInitialized` (lines 22-23). This phase removes those blocking awaits and adds lazy init guards to the affected services.
...
### Step 3: Clean up main.ts init flow
...
4. Keep only `import { neuInitialized } from "@/services/neutralino";` and `await neuInitialized;`
5. Keep `app.mount("#app")` before the await (correct Vue 3 behavior).
