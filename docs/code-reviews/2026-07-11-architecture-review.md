...
| #   | Candidate                                                                | Strength        | Files involved                                                                                                                      | Problem                                                                                                                                   |
| --- | ------------------------------------------------------------------------ | --------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Deepen entries store** -- split 6 concerns behind one seam             | Strong          | stores/entries.ts (352L), stores/clipboard.ts (114L), AppSidebar.vue, Tree.vue, EntryDetail.vue, EntryForm.vue, 7 dialog components | 32 exports spanning 6 concerns (tree, form state, clipboard buffer, skeleton UX, sort, CRUD). Interface nearly as wide as implementation. |
...
| 5   | **Sever lib->services dependency edge**                                  | Worth exploring | lib/store-walker.ts -> imports Fs from services, lib/shell.ts -> imports Fs from services                                           | Two lib modules import from services, inverting intended layering. Pure logic and I/O coupled.                                            |
...
- Explore candidate 1 (entries store deepening) -- grilling session to design the split
- Explore candidate 5 (lib->services reversal) -- can run in parallel
